...
$downloads = @(
    ...
    @{
        ...
        ExpectedSize = 4000000  # Approximate
    },
    @{
        Name = "esptool_py"
        Url = "https://github.com/espressif/esptool/releases/download/v5.1.0/esptool-v5.1.0-win64.zip"
        FileName = "esptool-v5.1.0-win64.zip"
        ExpectedSize = 5000000  # Approximate
    }
)
...
foreach ($dl in $downloads) {
    ...
    if (Test-Path $targetFile) {
        ...
        if ($fileSize -gt ($dl.ExpectedSize * 0.9)) {
            ...
            Write-Host "[PARTIAL] $($dl.Name) incomplete, re-downloading..." -ForegroundColor Yellow
    ...
    try {
        ...
        if ($job.JobState -eq "Transferred") {
            ...
            Write-Host "Error details: $($job.ErrorDescription)" -ForegroundColor Red
