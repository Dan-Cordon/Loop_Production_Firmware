...
Here is the high-level breakdown of how each file type works in the C++ architecture.
...
How it works: Every other file includes this header. When changing a pin or a speed setting, change it here once, and it automatically updates everywhere in the project.

Why: It prevents "magic numbers" buried deep in code (e.g., digitalWrite(4, HIGH) becoming digitalWrite(Config::RELAY_WATER_INLET, HIGH)).

2. src/types.h (The "Dictionary")
This file defines the shapes of data, but not the data itself.
...
Why: If defined int myVar = 0; in a header file, the compiler would try to create a new copy of myVar for every single file that included that header, causing "Multiple Definition" errors. This split ensures the variable is created exactly once but used everywhere.
...
Why: It keeps main task loops clean. Instead of 50 lines of bit-shifting code inside the main loop, you just see send_step_move_ack(motor_id). It also isolates hardware-specific code (like ESP32 RMT drivers), making it easier to port to STM32 later.

5. src/tasks.h & .cpp (The "Workers")
This is where the actual active behavior of the firmware lives.
...
Why: It separates concurrent activities. The CAN bus listener doesn't need to know how the valve controller works; it just updates the global state, and the valve task reacts to that change independently.
