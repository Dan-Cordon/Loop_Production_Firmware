...
template <typename T, size_t N> class SimpleMovingAverage {
private:
  ...
  size_t count =
      0; // The number of readings added so far (handles the initial fill)
...
public:
  ...
  void add(T new_reading) {
    ...
    readings[index] =
        new_reading;         // Overwrite the oldest reading with the new one
  ...
  T getAverage() const {
    if (count == 0)
      return 0; // Prevent division by zero if no readings have been added
    return sum /
           count; // The average is the sum divided by the number of readings
  }
};
...
constexpr adc2_channel_t ADC_VARIABLE_RESISTANCE = ADC2_CHANNEL_2; // GPIO 13

// Variable Resistance Sensor Configuration (0-190 Ohm)
constexpr int ADC_RESOLUTION = 4095;   // 12-bit ADC (0-4095)
constexpr float ADC_VREF = 3.3;        // Voltage the ADC compares against
constexpr float SENSOR_VIN = 5.0;      // Voltage powering the sensor divider
constexpr float R_FIXED = 220.0;       // Fixed resistor in Ohms
...
enum class MotorCmd : uint8_t { Off = 0, PanelSpeed = 1, FullSpeed = 10 };
...
constexpr int PWM_CONTROL_LOOP_INTERVAL_MS =
    20;                            // Faster loop for responsive PWM
...
constexpr size_t NOZZLE_UI_FILTER_SIZE =
    10; // Number of samples to average for the UI display.

constexpr float MCM_MM_PER_REV[static_cast<size_t>(Config::Motor::Count)] = {
...
struct TreatmentInput {
  ...
  float ccv_m = 0.0f, ccv_c = 0.0f; // m = Slope, c = Offset
...
struct ValveController {
  ...
  gpio_num_t active_relay_pin =
      GPIO_NUM_NC; // GPIO_NUM_NC indicates no pin is active
};
...
static inline float read_f32_le(const uint8_t *b) {
  float f;
  memcpy(&f, b, 4);
  return f;
}
...
float calculate_pressure(int raw_value) {
  ...
  float scaled_value =
      (float)(raw_value - ADC_MIN_RAW) / (float)(ADC_MAX_RAW - ADC_MIN_RAW);
...
void update_valve_pwm(ValveController &controller) {
  ...
  if (controller.active_relay_pin != GPIO_NUM_NC) {
    if (current_time - controller.window_start_time >=
        controller.activation_duration_ms) {
  ...
  if (current_time - controller.window_start_time <
      Config::PWM_ACTIVATION_WINDOW_MS) {
    return;
  }
  ...
  controller.activation_duration_ms =
      (Config::PWM_ACTIVATION_WINDOW_MS * abs(output_percent)) / 100.0;
  ...
  if (controller.direction) {
    ...
    controller.active_relay_pin = needs_pressure_increase
                                      ? controller.relayOpenPin
                                      : controller.relayClosePin;
  } else {
...
void checkStabilization() {
  ...
  if (!CanMixer::Config::waitingForPressure)
    return;
...
void valve_control_task(void *pv) {
  while (true) {
    ...
    //             state.nozzleController.setpoint,
    //             state.nozzleController.input,
    //             state.systemController.setpoint,
    //             state.systemController.input);
...
float to_kg_per_ha(float value, uint16_t unit) {
  ...
  case 2:
    return value * 28.3495f / 1000.0f;
  case 3:
    return value * DENSITY_G_PER_ML;
  case 4:
    return value * DENSITY_G_PER_ML / 1000.0f;
  default:
    return 0.0f;
  }
}
...
float computeRPM(const TreatmentInput &t) {
  if (!t.loaded || t.paused || t.rate_kg_per_ha <= 0.0f)
    return 0.0f;
  if (state.session.targetSpeed_mps <= 0 || state.session.rowSpacing_m <= 0 ||
      state.session.numFaces <= 0)
    return 0.0f;
  const float areaRate = state.session.targetSpeed_mps *
                         state.session.rowSpacing_m *
                         (float)state.session.numFaces;
  const float chemRate_gps = t.rate_kg_per_ha * (areaRate / 10.0f);
  // Serial.printf("Speed: %.2F, RowSpacing: %.2f, NumFaces: %d, kg/ha: %.2f,
  // CCV_M: %.2f, CCV_C: %.2f   ", state.session.targetSpeed_mps,
  //               state.session.rowSpacing_m, state.session.numFaces,
  //               t.rate_kg_per_ha, t.ccv_m, t.ccv_c);

  // Linear Regression: RPM = m * Rate + c
  // Ensure m and Rate are compatible. Assuming m is [RPM / (g/s)] and c is
  // [RPM].
  float rpm = (t.ccv_m * chemRate_gps) + t.ccv_c;
...
void processDoserStatus(uint8_t motor_index, uint16_t status_code) {
  if (motor_index >= static_cast<size_t>(Config::Motor::Count))
    return;
...
void sendHopperStatus(uint8_t motor_index, uint16_t status_code) {
  twai_message_t msg = {};
  msg.identifier =
      static_cast<uint32_t>(Config::CanID::HopperStatusBase) + motor_index;
...
/**
 * @brief Streams step move commands to any motor that hasn't been acknowledged
 * by the MCM.
 */
void executePrimingSequence() {
  for (uint8_t i = 0; i < static_cast<size_t>(Config::Motor::Count); ++i) {
    auto &motor = state.motors[i];
    // If motor is not loaded, is paused, has no volume to spray, OR has been
    // acknowledged, skip it. Serial.printf("MTR: %d, loaded %d, paused %d,
    // volperha %.2f, primeack %d",
    //               i, motor.loaded, motor.paused, state.session.sprayVolHa_L,
    //               motor.primingAckReceived);
    if (!motor.loaded || motor.paused || state.session.sprayVolHa_L <= 0 ||
        motor.primingAckReceived)
      continue;
    ...
    if (is_liquid && Config::LIQUID_ML_PER_DEG > 0) {
      ...
      // Updated Priming Logic for Linear Model: RPM = m * Rate + c
      // We drive at PRIMING_RPM (50).
      // Rate at this RPM: Rate = (PRIMING_RPM - c) / m
      // Time to dispense mass: Time = Mass / Rate
      // Degrees = Time * (PRIMING_RPM * 6 degrees/sec)

      if (abs(motor.ccv_m) > 0.0001f) { // Avoid divide by zero
        float rate_at_prime = (Config::PRIMING_RPM - motor.ccv_c) / motor.ccv_m;
        if (rate_at_prime > 0) {
          uint8_t solid_idx = i - static_cast<uint8_t>(Config::Motor::Solid1);
          float mass_g =
              priming_volume_mL * Config::SOLID_DENSITY_G_PER_ML[solid_idx];

          float time_s = mass_g / rate_at_prime;
          degrees_to_turn = time_s * (Config::PRIMING_RPM * 6.0f);
          // Serial.printf("DETAILS: Motor %d, mass: %.2f g, Rate@Prime: %.4f,
          // Time: %.2f s, Deg: %.2f\n", i, mass_g, rate_at_prime, time_s,
          // degrees_to_turn);
        }
      }
    }
    ...
    if (degrees_to_turn > 0) {
      ...
      if (twai_transmit(&msg, pdMS_TO_TICKS(20)) == ESP_OK) {
        ...
        // Serial.printf("WARN: Failed to send STEP MOVE command for motor
        // %d\n", i);
      }
    }
  }
...
/**
 * @brief [CORRECTED] Computes RPM for each motor and sends individual CAN
 * messages.
 */
...
void sendCleaningRPMs() {
  ...
  for (uint8_t i = static_cast<uint8_t>(Config::Motor::Liquid1);
       i <= static_cast<uint8_t>(Config::Motor::Liquid4); i++) {
    msg.identifier = static_cast<uint32_t>(Config::CanID::MotorRpmBase) + i;
    if (twai_transmit(&msg, pdMS_TO_TICKS(10)) != ESP_OK) {
...
/**
 * @brief [CORRECTED] Sends an explicit RPM=0 command to all motors
 * individually.
 */
    ...
    if (twai_receive(&msg, pdMS_TO_TICKS(100)) == ESP_OK) {
      ...
      case Config::CanID::DoserStatusSolids: {
        if (msg.data_length_code >= 2)
          processDoserStatus(static_cast<uint8_t>(Config::Motor::Solid1),
                             read_u16_le(&msg.data[0]));
        if (msg.data_length_code >= 4)
          processDoserStatus(static_cast<uint8_t>(Config::Motor::Solid2),
                             read_u16_le(&msg.data[2]));
        break;
      }
      case Config::CanID::DoserStatusLiquids: {
        if (msg.data_length_code >= 2)
          processDoserStatus(static_cast<uint8_t>(Config::Motor::Liquid1),
                             read_u16_le(&msg.data[0]));
        if (msg.data_length_code >= 4)
          processDoserStatus(static_cast<uint8_t>(Config::Motor::Liquid2),
                             read_u16_le(&msg.data[2]));
        if (msg.data_length_code >= 6)
          processDoserStatus(static_cast<uint8_t>(Config::Motor::Liquid3),
                             read_u16_le(&msg.data[4]));
        if (msg.data_length_code >= 8)
          processDoserStatus(static_cast<uint8_t>(Config::Motor::Liquid4),
                             read_u16_le(&msg.data[6]));
        break;
      }
      case Config::CanID::MixerStatus: {
        if (msg.data_length_code >= 2) {
          auto new_status =
              static_cast<Config::MixerStatus>(read_u16_le(msg.data));
          if (new_status != state.mixerStatus) {
            ...
            // status_msg.identifier =
            // static_cast<uint32_t>(Config::CanID::MixerStatus);
            // status_msg.data_length_code = 2;
            // write_u16_le(status_msg.data,
            // static_cast<uint16_t>(state.mixerStatus));
      ...
      case Config::CanID::MixerTankWeight: {
        if (msg.data_length_code >= 4)
          state.current_tank_weight_kg = read_f32_le(msg.data);
        break;
      }
      ...
      case Config::CanID::S1_RateUnit: {
        if (msg.data_length_code >= 6) {
          ...
          state.motors[static_cast<uint8_t>(Config::Motor::Solid1)]
              .rate_kg_per_ha = to_kg_per_ha(rate_val, unit);
          // Serial.printf("TREATMENT DATA: Motor S1, Rate: %.2f kg/ha (Raw:
          // %.2f, Unit: %d)\n",
          // state.motors[static_cast<uint8_t>(Config::Motor::Solid1)].rate_kg_per_ha,
          // rate_val, unit);
        }
        break;
      }
      case Config::CanID::S1_Ccv: {
        if (msg.data_length_code >= 8) {
          state.motors[static_cast<uint8_t>(Config::Motor::Solid1)].ccv_c =
              read_f32_le(&msg.data[0]); // A = Offset (c)
          state.motors[static_cast<uint8_t>(Config::Motor::Solid1)].ccv_m =
              read_f32_le(&msg.data[4]); // B = Slope (m)
          // Serial.printf("TREATMENT DATA: Motor S1, CCV M: %.2f, C: %.2f\n",
          // state.motors[static_cast<uint8_t>(Config::Motor::Solid1)].ccv_m,
          // state.motors[static_cast<uint8_t>(Config::Motor::Solid1)].ccv_c);
        }
        break;
      ...
      case Config::CanID::S2_RateUnit: {
        if (msg.data_length_code >= 6) {
          float rate_val = read_f32_le(&msg.data[0]);
          uint16_t unit = read_u16_le(&msg.data[4]);
          state.motors[static_cast<uint8_t>(Config::Motor::Solid2)]
              .rate_kg_per_ha = to_kg_per_ha(rate_val, unit);
          // Serial.printf("TREATMENT DATA: Motor S2, Rate: %.2f kg/ha (Raw:
          // %.2f, Unit: %d)\n",
          // state.motors[static_cast<uint8_t>(Config::Motor::Solid2)].rate_kg_per_ha,
          // rate_val, unit);
        }
        break;
      }
      case Config::CanID::S2_Ccv: {
        if (msg.data_length_code >= 8) {
          state.motors[static_cast<uint8_t>(Config::Motor::Solid2)].ccv_c =
              read_f32_le(&msg.data[0]);
          state.motors[static_cast<uint8_t>(Config::Motor::Solid2)].ccv_m =
              read_f32_le(&msg.data[4]);
          // Serial.printf("TREATMENT DATA: Motor S2, CCV M: %.2f, C: %.2f\n",
          // state.motors[static_cast<uint8_t>(Config::Motor::Solid2)].ccv_m,
          // state.motors[static_cast<uint8_t>(Config::Motor::Solid2)].ccv_c);
        }
        break;
      ...
      case Config::CanID::L1_RateUnit: {
        if (msg.data_length_code >= 6) {
          float rate_val = read_f32_le(&msg.data[0]);
          uint16_t unit = read_u16_le(&msg.data[4]);
          state.motors[static_cast<uint8_t>(Config::Motor::Liquid1)]
              .rate_kg_per_ha = to_kg_per_ha(rate_val, unit);
          // Serial.printf("TREATMENT DATA: Motor L1, Rate: %.2f kg/ha (Raw:
          // %.2f, Unit: %d)\n",
          // state.motors[static_cast<uint8_t>(Config::Motor::Liquid1)].rate_kg_per_ha,
          // rate_val, unit);
        }
        break;
      }
      ...
      case Config::CanID::L2_RateUnit: {
        if (msg.data_length_code >= 6) {
          float rate_val = read_f32_le(&msg.data[0]);
          uint16_t unit = read_u16_le(&msg.data[4]);
          state.motors[static_cast<uint8_t>(Config::Motor::Liquid2)]
              .rate_kg_per_ha = to_kg_per_ha(rate_val, unit);
          // Serial.printf("TREATMENT DATA: Motor L2, Rate: %.2f kg/ha (Raw:
          // %.2f, Unit: %d)\n",
          // state.motors[static_cast<uint8_t>(Config::Motor::Liquid2)].rate_kg_per_ha,
          // rate_val, unit);
        }
        break;
      }
      ...
      case Config::CanID::L3_RateUnit: {
        if (msg.data_length_code >= 6) {
          float rate_val = read_f32_le(&msg.data[0]);
          uint16_t unit = read_u16_le(&msg.data[4]);
          state.motors[static_cast<uint8_t>(Config::Motor::Liquid3)]
              .rate_kg_per_ha = to_kg_per_ha(rate_val, unit);
          // Serial.printf("TREATMENT DATA: Motor L3, Rate: %.2f kg/ha (Raw:
          // %.2f, Unit: %d)\n",
          // state.motors[static_cast<uint8_t>(Config::Motor::Liquid3)].rate_kg_per_ha,
          // rate_val, unit);
        }
        break;
      }
      ...
      case Config::CanID::L4_RateUnit: {
        if (msg.data_length_code >= 6) {
          float rate_val = read_f32_le(&msg.data[0]);
          uint16_t unit = read_u16_le(&msg.data[4]);
          state.motors[static_cast<uint8_t>(Config::Motor::Liquid4)]
              .rate_kg_per_ha = to_kg_per_ha(rate_val, unit);
          // Serial.printf("TREATMENT DATA: Motor L4, Rate: %.2f kg/ha (Raw:
          // %.2f, Unit: %d)\n",
          // state.motors[static_cast<uint8_t>(Config::Motor::Liquid4)].rate_kg_per_ha,
          // rate_val, unit);
        }
        break;
      }
      ...
      case Config::CanID::WaterInletValve: {
        if (msg.data_length_code >= 2) {
          ...
          if (temp != state.waterinletvalve) {
            ...
            gpio_set_level(Config::RELAY_WATER_INLET_CLOSE,
                           !state.waterinletvalve);
            gpio_set_level(Config::RELAY_WATER_INLET_OPEN,
                           state.waterinletvalve);
          }
        }
        break;
      ...
      case Config::CanID::PropSystemPressure: {
        if (msg.data_length_code >= 2) {
          ...
          if (temp != state.propsystempressure) {
            ...
            gpio_set_level(state.systemController.relayClosePin,
                           !state.propsystempressure);
            gpio_set_level(state.systemController.relayOpenPin,
                           state.propsystempressure);
          }
        }
        break;
      ...
      case Config::CanID::PropNozzlePressure: {
        if (msg.data_length_code >= 2) {
          ...
          if (temp != state.propnozzlepressure) {
            ...
            gpio_set_level(state.nozzleController.relayClosePin,
                           !state.propnozzlepressure);
            gpio_set_level(state.nozzleController.relayOpenPin,
                           state.propnozzlepressure);
          }
        }
        break;
...
void main_control_task(void *pv) {
  ...
  cleaning_status.identifier =
      static_cast<uint32_t>(Config::CanID::CleaningCycles);
  ...
  while (true) {
    ...
    if (millis() - lastHeartbeatTime > Config::HEARTBEAT_TIMEOUT_MS) {
      ...
      lastHeartbeatTime =
          millis(); // Reset timer to prevent continuous triggering
    }
    ...
    case Config::MixerStatus::Stopped: {
      ...
      // [CRITICAL FIX]: Ensure System Controller is ENABLED to maintain
      // setpoint
    ...
    case Config::MixerStatus::Priming:
      if (!state.priming_initiated) {
        resetPrimingAcks();
        for (uint8_t i = 0; i < static_cast<size_t>(Config::Motor::Count);
             ++i) {
      ...
      if (state.fill_state != FillState::MAINTAINING)
        break;
      // Serial.println("PRIMING INIT");

      executePrimingSequence(); // Continuously stream commands until
                                // acknowledged

      for (uint8_t i = 0; i < static_cast<size_t>(Config::Motor::Count); ++i) {
        auto &motor = state.motors[i];
        if (motor.loaded && !motor.paused && motor.priming_complete &&
            !motor.priming_status_sent) {
    ...
    case Config::MixerStatus::Running:
      if (state.priming_initiated == false) {
        ...
        if (!CanMixer::Config::waitingForPressure &&
            !state.nozzleController.enabled) {
    ...
    case Config::MixerStatus::StartResumeCleaning: {
      ...
      if (cleaningStateInstance.status <= 410) {
        twai_transmit(&cleaning_status, pdMS_TO_TICKS(10));
        if (cleaningStateInstance.status % 100 == 0 ||
            cleaningStateInstance.status % 100 == 60) // Purge cycle
        {

          if (cleaningStateInstance.visited_flag == false) {
            state.fill_state = FillState::IDLE; // Idle
            cleaningStateInstance.status =
                (cleaningStateInstance.iter + 1) * 100; // Purge in progress
            write_u16_le(&cleaning_status.data[0],
                         cleaningStateInstance.status);
            // Stop water intake
            gpio_set_level(Config::RELAY_WATER_INLET_CLOSE, 1);
            gpio_set_level(Config::RELAY_WATER_INLET_OPEN, 0);
            ...
            cleaningStateInstance.visited_flag =
                true; // restrict the valve to be actuated over time
          }
          if (state.current_tank_weight_kg <= 1) // Tank is empty
          {
            cleaningStateInstance.status =
                (cleaningStateInstance.iter + 1) * 100 + 10; // Water is empty
            write_u16_le(&cleaning_status.data[0],
                         cleaningStateInstance.status);
            cleaningStateInstance.visited_flag = false;
          }
        } else if (cleaningStateInstance.status % 100 == 10 ||
                   cleaningStateInstance.status % 100 == 50) // Refill Cycle
        {
          if (!cleaningStateInstance.visited_flag &&
              cleaningStateInstance.status < 410) {
            state.fill_state = FillState::FILLING; // Idle
            cleaningStateInstance.status =
                (cleaningStateInstance.iter + 1) * 100 +
                50; // Refill in progress
            write_u16_le(&cleaning_status.data[0],
                         cleaningStateInstance.status);
          ...
          {
            cleaningStateInstance.status =
                (cleaningStateInstance.iter + 1) * 100 + 60;
            write_u16_le(&cleaning_status.data[0],
                         cleaningStateInstance.status);
    ...
    case Config::MixerStatus::StopCleaning: // Stop water inlet and nozzle exit
                                            // valves
    {
...
void sensor_task(void *pv) {
  ...
  while (true) {
    ...
    state.nozzleController.input =
        (CanMixer::Config::filterAlpha * rawNozzlePressure) +
        (1.0 - CanMixer::Config::filterAlpha) * state.nozzleController.input;
    ...
    state.systemController.input =
        (CanMixer::Config::filterAlpha * rawSystemPressure) +
        (1.0 - CanMixer::Config::filterAlpha) * state.systemController.input;
...
void tank_fill_task(void *pv) {
  while (true) {
    ...
    case FillState::MAINTAINING:
      if (current_level_L <
          (Config::TANK_FILL_TARGET_L - Config::TANK_MAINTAIN_HYSTERESIS_L)) {
        // Open the valve
        gpio_set_level(Config::RELAY_WATER_INLET_CLOSE, 0);
        gpio_set_level(Config::RELAY_WATER_INLET_OPEN, 1);
...
void spray_valve_task(void *pv) {
  while (true) {
    if (int(state.mixerStatus) == 1000)
      continue;
    // The spray outlet should be open only when the mixer is in a spraying
    // state
    bool should_be_spraying =
        (state.mixerStatus == Config::MixerStatus::Running ||
         state.mixerStatus == Config::MixerStatus::Rundown ||
         state.mixerStatus == Config::MixerStatus::CleaningPurge ||
         (cleaningStateInstance.status % 100 == 0 &&
          cleaningStateInstance.status > 0) ||
...
// =================================================================================
// Variable Resistance Sensor Helper Function
// =================================================================================

float calculate_resistance_ohms(int raw_adc) {
  // Linear mapping from ADC raw values to resistance
  // Calibration: ADC 12 = 0Ω, ADC 187 = 190Ω
  const int ADC_MIN = 14;
  const int ADC_MAX = 187;
  const float RESISTANCE_MAX = 190.0f;
  
  if (raw_adc <= ADC_MIN) return 0.0f;
  if (raw_adc >= ADC_MAX) return RESISTANCE_MAX;
  
  float r_variable = (raw_adc - ADC_MIN) * (RESISTANCE_MAX / (ADC_MAX - ADC_MIN));
  return r_variable;
}
...
void gpio_monitor_task(void *pv) {
  int raw_resistance = 0;
  
  while (true) {
    int gpio17_state = gpio_get_level(GPIO_NUM_17);
    int gpio18_state = gpio_get_level(GPIO_NUM_18);
    adc2_get_raw(CanMixer::Config::ADC_VARIABLE_RESISTANCE, ADC_WIDTH_BIT_12, &raw_resistance);
    float resistance = calculate_resistance_ohms(raw_resistance);
    
    Serial.print("GPIO17: ");
    Serial.print(gpio17_state);
    Serial.print(" | GPIO18: ");
    Serial.print(gpio18_state);
    Serial.print(" | ADC Raw: ");
    Serial.print(raw_resistance);
    Serial.print(" | Resistance: ");
    Serial.print(resistance, 2);
    Serial.print(" Ohms | Tank Weight: ");
    Serial.print(CanMixer::state.current_tank_weight_kg, 2);
    Serial.println(" kg");
...
void setup() {
  ...
  state.motors[static_cast<uint8_t>(Config::Motor::Liquid1)].ccv_m = 550.0f;
  state.motors[static_cast<uint8_t>(Config::Motor::Liquid1)].ccv_c = 0.0f;
  state.motors[static_cast<uint8_t>(Config::Motor::Liquid2)].ccv_m = 550.0f;
  state.motors[static_cast<uint8_t>(Config::Motor::Liquid2)].ccv_c = 0.0f;
  state.motors[static_cast<uint8_t>(Config::Motor::Liquid3)].ccv_m = 550.0f;
  state.motors[static_cast<uint8_t>(Config::Motor::Liquid3)].ccv_c = 0.0f;
  state.motors[static_cast<uint8_t>(Config::Motor::Liquid4)].ccv_m = 550.0f;
  state.motors[static_cast<uint8_t>(Config::Motor::Liquid4)].ccv_c = 0.0f;
  // Serial.println(
  //     "SETUP: Hardcoded CCV of 3300.0 applied to all liquid dosers.");
  ...
  gpio_set_direction(Config::RELAY_WATER_INLET_OPEN,
                     GPIO_MODE_OUTPUT); // tank inlet
  gpio_set_direction(Config::RELAY_WATER_INLET_CLOSE, GPIO_MODE_OUTPUT);
  gpio_set_direction(Config::RELAY_SPRAY_OUTLET_OPEN,
                     GPIO_MODE_OUTPUT); // spray through nozzles
  gpio_set_direction(Config::RELAY_SPRAY_OUTLET_CLOSE, GPIO_MODE_OUTPUT);

  // GPIO14 configured as constant ON output
  gpio_set_direction(GPIO_NUM_14, GPIO_MODE_OUTPUT);
  gpio_set_level(GPIO_NUM_14, 1);

  // GPIO17 and GPIO18 configured as pullup inputs for capacitive sensor
  ...
  adc2_config_channel_atten(Config::ADC_VARIABLE_RESISTANCE, ADC_ATTEN_DB_11);

  // CAN setup
  twai_general_config_t g_config = TWAI_GENERAL_CONFIG_DEFAULT(
      Config::CAN_TX_PIN, Config::CAN_RX_PIN, TWAI_MODE_NORMAL);
  ...
  xTaskCreatePinnedToCore(valve_control_task, "ValveControl", 2048, nullptr, 7,
                          nullptr, 1);
  xTaskCreatePinnedToCore(main_control_task, "MainControl", 4096, nullptr, 5,
                          nullptr, 1);
  xTaskCreatePinnedToCore(tank_fill_task, "TankFill", 4096, nullptr, 4, nullptr,
                          1);
  xTaskCreatePinnedToCore(spray_valve_task, "SprayValve", 2048, nullptr, 4,
                          nullptr, 1);
  xTaskCreatePinnedToCore(gpio_monitor_task, "GPIOMonitor", 2048, nullptr, 3,
                          nullptr, 1);
}
